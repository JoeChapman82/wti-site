...
async function backupDb() {
    ...
    try {
        ...
        console.log(`Backing up ${allUsers.length} users and ${allRecords.length} records.`)
        const userS3 = new AWS.S3();
        let params = { Body: JSON.stringify(allUsers), Bucket: bucket, Key: userKey };
        const userData = await userS3.upload(params).promise();
        console.log('User backup complete:', userData);
        const recordS3 = new AWS.S3();
        params = { Body: JSON.stringify(allRecords), Bucket: bucket, Key: recordKey };
