...
module.exports = async (req, res, next) => {
    ...
    if(req.body.reportType === 'time') {
        ...
        queries.push(findRecords.count({dateOfRescue: {$gte: start, $lte: end}, class: 'mammals'}));
        queries.push(findRecords.count({dateOfRescue: {$gte: start, $lte: end}, class: 'aves'}));
        queries.push(findRecords.count({dateOfRescue: {$gte: start, $lte: end}, class: 'reptiles'}));
        ...
        if(req.body.identityName) {
            ...
            finalDispositionStates.forEach((state) => {
                queries.push(findRecords.count({identityName: req.body.identityName, finalDisposition: state}));
            });
            ...
            finalDispositionStates.forEach((state) => {
                queries.push(findRecords.count({groupName: req.body.groupName, finalDisposition: state}));
            });
            queryNames.push('total', ...finalDispositionStates);
        } else {
            queries.push(findRecords.count({class: req.body.class}));
            finalDispositionStates.forEach((state) => {
                queries.push(findRecords.count({class: req.body.class, finalDisposition: state}));
            });
            queryNames.push('total', ...finalDispositionStates);
        }
        ...
        queryNames.push('deadOnArrival', 'diedUnderCare', 'Released', 'Transferred', 'Escaped');
    }
    try {
