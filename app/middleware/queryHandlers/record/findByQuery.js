...
module.exports = async (req, res, next) => {  
    ...
    Object.entries(req.body).forEach(([key, value]) => {
        ...
        if(typeof value === 'string' && value.length > 0 && acceptedQueryFields.includes(key)) {
            if(!nonStringFields.includes(key) && !nonStringFields.includes(key)) {
                toQuery[key] = { '$regex': value, '$options': 'i' };
                queryParams[key] = value;
            } else if(key === 'dateAdded') {
                toQuery[key] = { '$gte': new Date(`${value} 00:00:00`),  '$lte': new Date(`${value} 23:59:59`)};
    ...
    if(req.originalUrl === '/admin/existing-cases' && Object.keys(toQuery).length > 0 && !req.body.hasAtLeastOneSubmit) {
        start = 0;
    };
    const end = recordCount - start < recordRetrievalLimit ?
        recordCount :
        (start - 1) + recordRetrievalLimit;
    res.locals.lastUpdatedAt = new Date();
    res.locals.lastUpdatedAt = new Date();

    find.byQuery(toQuery, start, recordRetrievalLimit)
        .then(response => {
