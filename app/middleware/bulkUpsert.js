		...
		if (lineNumber > 3) {
			...
			if (values.length > 2 && isAcceptableLine) {
				for (let i = 0; i < values.length; i++) {
					...
					if (
						uploadMap[keys[i]] &&
						typeof uploadMap[keys[i]].method === 'function'
					) {
						...
						if (
							uploadMap[keys[i]].linkedField === 'remarks' &&
							values[i][0] === '"'
						) {
							recordToUpdate[uploadMap[keys[i]].linkedField] =
				...
				let additionalData = animalData.find(
			...
			for (let i = 0; i < additionalKeyInfo.length; i++) {
				if (additionalKeyInfo[i].trim().length > 0) {
					if (additionalKeyInfo[i].trim() === 'Lat') {
						...
						keys[i] = `${keys[i - 1].substring(0, keys[i - 1].length - 4)} ${
							additionalKeyInfo[i]
						}`;
					}
				}
