...
module.exports = (req, res, next) => {
    ...
    let matchedPath = Object.keys(schemaMap).reverse().find((key) => new RegExp(key).test(req.originalUrl.split('?')[0]));
    ...
    validatorKeys.forEach((key) => {
        validationSchema.validators[key].forEach((validation) => {
            ...
            if(validation.isConditional) {
                validation.conditions.forEach((condition) => {
                    if(condition.value) {
                        ...
                        if(!validators[condition.validator](req.body[key], ...validation.options || [], req.body[validation.comparitor])) {
                            conditionMet = false;
                        }
                    }
            ...
            if(conditionMet && !validationMethod(req.body[key], ...validation.options || [], req.body[validation.comparitor])) {
